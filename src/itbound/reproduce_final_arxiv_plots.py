...
def _resolve_root() -> Path:
    ...
    if (here / "experiments").exists() or (here / "src" / "experiments").exists():
        return here
    cwd = Path.cwd()
    if (cwd / "experiments").exists() or (cwd / "src" / "experiments").exists():
        return cwd
    return here
...
def _run_plot_n(*, plot_name: str, artifact_path: Path, dest_path: Path, dry_run: bool) -> None:
...
def _run_plot_ribbon(
    *, plot_name: str, summary_path: Path, artifact_path: Optional[Path], dest_path: Path, dry_run: bool
) -> None:
    outdir = dest_path.parent
    cmd = [
        sys.executable,
...
def _run_plot_idhp(*, plot_name: str, artifact_path: Path, dest_path: Path, dry_run: bool) -> None:
    outdir = dest_path.parent
    cmd = [
        sys.executable,
